<script lang="ts">
  ...
  import { MOBILE_BREAKPOINT, VEHICLE_NOTICE_DISMISSED_COOKIE } from '$lib/constants';
  import { innerWidth } from 'svelte/reactivity/window';
  import { onMount } from 'svelte';
  import { storeDismissal, wasDismissed } from './membership-notice';
  ...
  interface Props {
    ...
    onShowMobileMembershipNotice?: () => void;
    onBecomeMember?: () => void;
  }
  ...
  // Initiate with false to keep SSR & hydration consistent
  let showMemberNotice = $state(false);

  const toggleMemberNotice = (e?: Event | undefined) => {
    e?.preventDefault();
    e?.stopPropagation();
    if (isMobile) {
      onShowMobileMembershipNotice();
  ...
  onMount(() => {
    // Open the membership modal by default on desktop, when it was not dismissed yet,
    // and after the vehicle notice
    if (!isMobile && !wasDismissed() && getCookie(VEHICLE_NOTICE_DISMISSED_COOKIE)) {
      // since it defaults to false, this will show the notice
      toggleMemberNotice();
    }
  });
</script>
