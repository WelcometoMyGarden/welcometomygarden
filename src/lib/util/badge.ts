...
let badgeSupportCheckedAndGranted = false;
 ...
 *     Badge permission starts off as 'denied' before notification permissions are requested,
 *     and become 'prompt' after this.
 *
...
async function checkBadgeSupport(allowPotentialPromptForSupport = true): Promise<boolean> {
  ...
  if (badgeSupportCheckedAndGranted) return badgeSupported;
  ...
  try {
    ...
    if (display !== 'granted') {
      ...
      if (allowPotentialPromptForSupport && display !== 'denied') {
        logger.info('Prompting for badge permission, should normally not result in a prompt');
    ...
    badgeSupported = true;
    // Once it's granted, we cache the result
    badgeSupportCheckedAndGranted = true;
