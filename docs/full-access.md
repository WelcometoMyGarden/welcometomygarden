...
The source images for this process live in `src/lib/assets`, and are tracked in the repo via [Git LFS](https://git-lfs.com/) — see [`.gitattributes`](../.gitattributes). Make sure Git LFS is installed (see [dev-env.md](./dev-env.md)). PNG/JPG/JPEG files under `src/lib/assets` are LFS-tracked; SVGs are kept as regular text-tracked files.
