...
3. Build the AAB via Build -> Generate Signed App Bundle ... Select the desired flavor. This will build into `android/app/{flavor}/{build_type}/app-{flavor}-{buildtype}.aab`
...
1. Select the desired project Scheme in the top status bar ("App" or "App Staging").
2. In the Project navigator (first left top) -> App (left top) -> Targets -> App (left) -> Build Settings (top), go to the Versioning section, adjust "Current Project Version" and "Marketing Version" as appropriate.
3. Go to Product -> Archive in the menu, which will rebuild the app if necessary and create a bundle/archive (to see existing archives, open the Organizer in Window)
4. Click "Distribute app". It's also possible to first "Validate app" to make sure there are no submission errors before distributing.
