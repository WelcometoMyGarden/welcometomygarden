...
class OfflineGateViewController: CAPBridgeViewController {
    ...
    private func showOverlay() {
        ...
        let green = UIColor(red: 0x49 / 255.0, green: 0x57 / 255.0, blue: 0x47 / 255.0, alpha: 1)
        ...
        // Spinner (no caption), centered below the band.
        let spinner = UIActivityIndicatorView(style: .medium)
        spinner.color = green
        spinner.startAnimating()

        let spinnerStack = UIStackView(arrangedSubviews: [spinner])
        spinnerStack.axis = .vertical
        spinnerStack.alignment = .center
        spinnerStack.translatesAutoresizingMaskIntoConstraints = false

        let bottom = UIStackView(arrangedSubviews: [band, spinnerStack])
        ...
        NSLayoutConstraint.activate([
            ...
            // The band spans the full width; the spinner sits below it, above the safe-area bottom.
    ...
    private static func localized(_ key: StringKey) -> String {
        let messages: [String: String] = [
            "en": "You seem to be offline. To use WTMG, try to get back online.",
            "nl": "Je lijkt offline te zijn. Ga terug online om WTMG te gebruiken.",
            "fr": "Vous semblez hors ligne. Pour utiliser WTMG, reconnectez-vous à Internet.",
            "de": "Du scheinst offline zu sein. Um WTMG zu nutzen, geh wieder online.",
            "es": "Parece que no tienes conexión. Para usar WTMG, vuelve a conectarte a Internet."
        ]
