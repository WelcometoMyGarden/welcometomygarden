...
### iOS native dependencies (Swift Package Manager)

The iOS project uses SPM, not CocoaPods. There is no `Podfile` and no `.xcworkspace` —
build and open `ios/App/App.xcodeproj` directly.

- `ios/App/CapApp-SPM/Package.swift` is **generated by `cap sync`** — never edit it by
  hand. It lists one local package per Capacitor plugin, resolved out of `node_modules`.
- App-level native dependencies (currently `firebase-ios-sdk`, for `FirebaseCore` +
  `FirebaseMessaging`) live as package references **on the Xcode project**, and must be
  linked to _both_ the `App` and `App Staging` targets. Adding them to
  `CapApp-SPM/Package.swift` would be wiped on the next `cap sync`.
- `ios/debug.xcconfig` supplies `CAPACITOR_DEBUG` and is the base configuration of the
  **Debug** config of both targets. Release has no base configuration.
- A Capacitor plugin is only included if its npm package ships a `Package.swift`. One
